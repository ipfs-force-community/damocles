...
func NewSenderSelector(
	...
	cache := cache.New(
		time.Duration(
			policy.NetParams.BlockDelaySecs/2,
		),
		time.Duration(
			policy.NetParams.BlockDelaySecs,
		),
	)
...
func (s *SenderSelector) Select(
	...
	if !found {
		var err error
		sender, err, _ = s.group.Do(
			key,
			func() (any, error) {
				senderInner, errInner := s.selectInner(
					ctx,
					mid,
					senders,
				)
	...
	return sender.(address.Address), nil //revive:disable-line:unchecked-type-assertion
}

func (s *SenderSelector) selectInner(
	ctx context.Context,
	mid abi.ActorID,
	senders []address.Address,
) (address.Address, error) {
	...
	for i := range senders {
		if senders[i].Protocol() != address.ID {
			var err error
			senders[i], err = s.lookupID.StateLookupID(
				ctx,
				senders[i],
			)
			if err != nil {
	...
	maddr, err := address.NewIDAddress(
		uint64(mid),
	)
	if err != nil {
		return address.Undef, err
	}
	minerInfo, err := s.chain.StateMinerInfo(
		ctx,
		maddr,
		types.EmptyTSK,
	)
	if err != nil {
		return address.Undef, fmt.Errorf(
			"get miner info for %s: %w",
			maddr,
			err,
		)
	}
	minerAddrs := lo.Flatten(
		[][]address.Address{
			{minerInfo.Owner, minerInfo.Worker},
			minerInfo.ControlAddresses,
		},
	)
	validAddrs := lo.Intersect(
		senders,
		minerAddrs,
	)
	...
	if len(validAddrs) == 0 {
		return address.Undef, fmt.Errorf(
			"no valid sender found",
		)
	}

	balancesCache := make(
		map[address.Address]types.BigInt,
	)
	sort.Slice(validAddrs, func(i, j int) bool {
		balanceI := valueOrInsert(
			balancesCache,
			validAddrs[i],
			func() types.BigInt {
				return s.getBalance(
...
func (s *SenderSelector) getBalance(
	ctx context.Context,
	addr address.Address,
	defaultBalance types.BigInt,
) types.BigInt {
	actor, err := s.chain.StateGetActor(
		ctx,
		addr,
		types.EmptyTSK,
	)
...
func valueOrInsert[K comparable, V any](
	in map[K]V,
	key K,
	fallback func() V,
) V {
