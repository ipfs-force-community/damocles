...
fn utf8(buf: &[u8]) -> Result<&str, io::Error> {
    str::from_utf8(buf).map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Unable to codec input as UTF8"))
}

#[inline]
fn without_carriage_return(mut s: BytesMut) -> BytesMut {
        ...
        loop {
            ...
            match (self.is_discarding, newline_offset) {
                ...
                (false, Some(offset)) => {
                    ...
                    let line = line.split_to(buf.len() - 1);
...
impl Encoder<Bytes> for LinesCodec {
    type Error = LinesCodecError;

    fn encode(&mut self, line: Bytes, buf: &mut BytesMut) -> Result<(), LinesCodecError> {
        let line = utf8(&line)?;
