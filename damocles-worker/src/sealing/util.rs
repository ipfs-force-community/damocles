...
pub fn scan_memory_files(
    memory_file_dir_pattern: &MemoryFileDirPattern,
) -> Result<Vec<Vec<PathBuf>>> {
    use glob::glob;
    use regex::Regex;

    let re_numa_node_idx =
        Regex::new(memory_file_dir_pattern.to_regex_pattern().as_str())
    ...
    let numa_memory_files = match numa_memory_files_map.keys().max() {
        Some(&max_node_idx) => {
            ...
            for i in 0..=max_node_idx {
                numa_vec
                    .push(numa_memory_files_map.remove(&i).unwrap_or_default())
            }
            numa_vec
        }
        None => Vec::new(),
...
impl MemoryFileDirPattern {
    ...
    pub fn to_path(&self, numa_node_idx: u32) -> PathBuf {
        PathBuf::from(self.0.replacen(
            Self::NUMA_NODE_IDX_VAR_NAME,
            numa_node_idx.to_string().as_str(),
            1,
        ))
    }
}
...
mod tests {
    ...
    fn test_scan_memory_files() {
        const NUMA_NODE_IDX_VAR_NAME: &str =
            MemoryFileDirPattern::NUMA_NODE_IDX_VAR_NAME;
        ...
        let cases = vec![
            TestCase {
                memory_file_dir_pattern: format!(
                    "abc/numa_{}",
                    NUMA_NODE_IDX_VAR_NAME
                ),
                numa_node_files: vec![(0, 4), (1, 0), (3, 2)]
                    .into_iter()
                    .collect(),
            },
            TestCase {
            ...
            TestCase {
                ...
                numa_node_files: vec![(0, 4), (1, 2), (2, 3), (3, 2)]
                    .into_iter()
                    .collect(),
            },
            TestCase {
                ...
                numa_node_files: vec![(0, 0), (1, 0), (2, 0), (3, 0)]
                    .into_iter()
                    .collect(),
            },
            TestCase {
        ...
        for c in cases {
            ...
            let numa_node_num =
                *c.numa_node_files.keys().max().unwrap_or(&0) + 1;
            let mut expected_numa_memory_files = vec![vec![]; numa_node_num];
            for (numa_index, count) in c.numa_node_files {
                let dir = c.memory_file_dir_pattern.replacen(
                    NUMA_NODE_IDX_VAR_NAME,
                    numa_index.to_string().as_str(),
                    1,
                );
                expected_numa_memory_files[numa_index] = generated_random_files(
                    tempdir.path().join(dir.trim_matches('/')),
                    count,
                );
            }
            ...
            let p = MemoryFileDirPattern::new(
                &c.memory_file_dir_pattern,
                tempdir.path().to_str().unwrap(),
            );
            let mut actually_numa_memory_files =
                scan_memory_files(&p).expect("scan memory files must be ok");
            actually_numa_memory_files
                .iter_mut()
