...
我们可以在 venus-worker 配置的外部执行器配置组 ([[[processors.{stage_name}]]](./03.venus-worker的配置解析.md#processorsstage_name)) 中通过 `cgroup.cpuset` + `numa_preferred` 配置项限制 processor 仅使用该 NUMANode 中的 CPU, 内存也尽量优先从该 NUMANode 中分配, 可以提高 CPU 工作效率。
