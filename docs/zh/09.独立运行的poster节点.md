...
// 维护配置文件
venus-sector-manager --home ~/.venus-sector-manager2 daemon run --listen=":2789" --poster --miner
```
...
```
venus-sector-manager --home ~/.venus-sector-manager2 daemon run --listen=":2789" --conf-dir="~/.venus-sector-manager" --poster --miner
```
...
3. 初始化并启动独立 `PoSter` 节点
   ...
   venus-sector-manager --home=~/.venus-individual-poster daemon run --poster --miner --listen=":2789" --conf-dir="~/.venus-sector-manager" --ext-prover
   ```
