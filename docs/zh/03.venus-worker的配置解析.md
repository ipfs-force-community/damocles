...
`processors.limitation.staggered` 用于配置指定封装阶段并行任务的交错启动时间间隔。配置此项后当指定阶段有多个任务同时启动时，venus-worker 会依次根据配置的时间间隔启动任务，以避免任务同时启动造成磁盘 IO 等资源紧张的问题。
...
# 多个 pc1 任务交错启动的时间间隔，选填项，字符串类型 (e.g. "1s", "2min")
# pc1 = "5min"
# pc2 = "4min"
```

举例来说，如果设置了 `pc1 = "5min"`，当两个 pc1 任务同时启动时, 会先执行第一个任务 5 分钟执行第二个任务。
