...
* ~~`cgroup.cpuset` 中的 CPU cores 尽量不要跨 NUMA 节点~~ (venus-cluster v0.5.0 之后，支持加载 NUMA 亲和的 hugepage 内存文件，已经可以跨 NUMA 节点分配 cpuset 不会产生影响)
...
此时 CPU 的共享缓存数量为 2 (L3#0, L3#1), 并且配置 CPU cores 数量不一致，不满足 rust-fil-proofs 规则，无法启动。正确的配置可以是 `processors.pc2.cgroup.cpuset = "0-7"`。
