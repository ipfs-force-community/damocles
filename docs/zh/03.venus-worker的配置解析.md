...
v0.5.0 之后支持 `sealing_thread` 配置热更新。在指定的 `sealing_thread` 中的 `location` 目录下创建名为 `config.toml` 的热配置文件, 该配置文件的内容与 [`[[sealing_thread]]`](03.venus-worker的配置解析.md#sealing_thread) 内容完全一致，此配置文件中的配置项会覆盖 venus-worker 中对应的 `[[sealing_thread]]` 的配置项，并且修改此配置文件不需要重启 venus-worker 即可生效。

venus-worker 中的 `sealing_thread` 会在每次扇区任务开始之前检查 `location` 目录下的 `config.toml` 文件，如果 `config.toml` 文件的内容发生了变化或者此文件删除了都会重新加载或移除此文件的配置。
