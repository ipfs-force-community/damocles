...
* `cgroup.cpuset` 中的 CPU cores 数量与之对应的 CPU cache 数量必须是整数倍数关系

    [rust-fil-proofs](https://github.com/filecoin-project/rust-fil-proofs) 中规定, 当启用 `multicore sdr` 时, CPU 核心数量和这些 CPU 核心对应的 CPU cache 的数量必须是整数倍数关系。 如果外部执行器使用的是 rust-fil-proofs 或者基于 rust-fil-proofs 开发，则必须遵守这个规则，否则可以忽略。 venus-worker 默认的执行器是基于 rust-fil-proofs 开发的。

* `cgroup.cpuset` 中的 CPU cores 尽量不要跨 NUMA 节点
    
    跨 NUMA 节点会使得 CPU 访问内存速度变慢。 如果配置的 CPU cores 均在同一 NUMA 节点，可以将 `processors.{stage_name}.numa_preferred` 配置为对应的 NUMA 节点 id。
    

使用 [venus-worker-util](https://github.com/ipfs-force-community/venus-cluster/blob/main/docs/zh/12.venus-worker-util.md#hwinfo) 查看 CPU 信息
```shell
...
此时 CPU cache 数量为 2 (L3#0, L3#1), CPU cores 数量为 7， CPU cores 数量不是与之对应的 CPU cache 数量的整数倍，不满足 rust-fil-proofs 规则，无法启动。正确的配置可以是 `processors.pc1.cgroup.cpuset = "0-7"`。
