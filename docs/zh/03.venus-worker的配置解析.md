...
* `cgroup.cpuset` 中的 CPU cores 数量与之对应的 CPU 共享缓存数量必须是整数倍数关系
...
此时 CPU 的共享缓存数量为 2 (L3#0, L3#1), CPU cores 数量为 7， CPU cores 数量不是与之对应的 CPU 共享缓存数量的整数倍，不满足 rust-fil-proofs 规则，无法启动。正确的配置可以是 `processors.pc1.cgroup.cpuset = "0-7"`。
