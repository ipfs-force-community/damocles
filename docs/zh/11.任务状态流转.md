...
1. 对于一个已经报错暂停，且无法恢复的扇区封装任务，如 ticket 已过期，可以通过
   ...
   ```
   venus-worker worker -c <config file path> resume --state Aborted --index <index>
   ```

   或

   ```
   venus-sector-manager util worker resume <worker instance name or address> <thread index> Aborted
   ```
