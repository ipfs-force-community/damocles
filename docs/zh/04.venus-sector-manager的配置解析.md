...
```
### [Common.Plugins]
`Common.Plugins` 配置 venus-sector-manager 的插件存放路径
```toml
[Common.Plugins]
# 插件存放路径，选填项，字符串类型
# 默认为空字符串，表示不加载任何插件
...
# 默认为空字符串
# BaseDir 为空字符串时 venus-sector-manager 会使用 home dir(默认为 ~/.venus-sector-manager) 存放 Badger 数据库文件
#BaseDir = ""
...
#GasOverPremium = "0.0"
```
> `MaxWait` 并不是等待聚合消息个数的最大时间，它代表的含义是：
> - `Miners.Commitment.Pre` 是指扇区 `Ticket` 的超时时间，如扇区 `Ticket` 产生的时间为 `10:00:00`, 设置的  `MaxWait` 为 `02h0m0s`, 则 `12:00:00` 后的任何一次轮询都会触发此扇区 `precommit` 阶段消息的发送;
> - `Miners.Commitment.Prove` 是指扇区 `Seed` 的超时时间，判断逻辑同 `Ticket`；
> - `Miners.Commitment.Terminate` 是指扇区 `TerminateInfo.AddedHeight` 的超时时间，判断逻辑同 `Ticket`；
> 
