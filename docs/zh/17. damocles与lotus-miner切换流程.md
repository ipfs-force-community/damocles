...
导入扇区元数据是通过调用各自接口进行的，故过程中需要 `lotus-miner` 和 `damocles-manager` 服务都启动。其中 `<lotus-miner api>` 位于 ~/.loutsminer/api ，而 `<lotus-miner token>` 位于 `~/.loutsminer/token` 中。
```
./damocles-manager util sealer sectors import --api=<~/.loutsminer/api> \
--token=<~/.loutsminer/token>
```
可选 `flag`：
