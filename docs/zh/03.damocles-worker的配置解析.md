...
# 如果 "/path/to/your_local_pieces_dir/piece_file_name" 文件不存在, worker 也会从 damocles-manager 加载
...
`sealing_thread` 会继承 [`[sealing]`](https://github.com/ipfs-force-community/damocles/blob/main/docs/zh/03.damocles-worker%E7%9A%84%E9%85%8D%E7%BD%AE%E8%A7%A3%E6%9E%90.md#sealing) 中的配置。 可以使用 `sealing.*` 覆盖掉 `[sealing]` 的配置。
...
# 默认值为 false
# 自 v0.4.0 起，持久化存储分配逻辑转到 damocles-manager 上，此配置废弃
# enable_space_weighted = false
...
* `cgroup.cpuset` 中的 CPU cores 尽量不要跨 NUMA 节点， 跨 NUMA 节点会使得 CPU 访问内存速度变慢 (**damocles v0.5.0 之后，[支持加载 NUMA 亲和的 hugepage 内存文件](./15.damocles-worker_PC1_HugeTLB_Pages_支持.md)，如果启用该功能可以跨 NUMA 节点分配 cpuset 不会产生影响**)
