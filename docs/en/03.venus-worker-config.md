...
* For PC1 external processors with multicore SDR enabled, `cgroup.cpuset` must be configured with the entirety of CPU cores under one L3 cache as much as possible. If you need to configure portion of CPU cores under one L3 cache, you must make sure that the number of CPU cores under each L3 cache is the same.
...
* ~~ The CPU cores in `cgroup.cpuset` should not span NUMA nodes as much as possible ~~ (After `venus-cluster` v0.5.0, if system supports supports loading NUMA friendly `hugepage` memory files, then you can allocate `cpusets` across NUMA nodes without concerns)
