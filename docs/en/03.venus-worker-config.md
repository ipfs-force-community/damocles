...
* For PC1 external processors with multicore SDR enabled, `cgroup.cpuset` must use the entirety of the CPU cores under one L3 cache as a base unit for configuration as much as possible. If you need to configure portion of CPU cores under one L3 cache, you must make sure that the number of CPU cores under each L3 cache is the same.
...
* ~~The CPU cores in `cgroup.cpuset` should not span NUMA nodes as much as possible~~ (After `venus-cluster` v0.5.0, if system supports loading NUMA affinity `hugepage` memory files, then you can allocate `cpusets` across NUMA nodes without concerns)

    ~~Across NUMA nodes will make CPU access to memory slower. If the configured CPU cores are on the same NUMA node, `processors.{stage_name}.numa_preferred` can be configured with the corresponding NUMA node id.~~
