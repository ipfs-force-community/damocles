...
When assigning cpu cores to `cgroup.cpuset`, either all or none of cores under same L3 cache must be assigned. In addition, the total number of CPU cores assigned to `cgroup.cpuset` must be an integer multiple of cores in L3 cache (which is 4 in the above example).

In the above example, the number of core in CPU caches is 4, the number of CPU cores is 7, which means that it does not satisfy the rust-fil-proofs rule, thus cannot be started. The correct configuration could be `processors.pc1.cgroup.cpuset = "0-7"`.
