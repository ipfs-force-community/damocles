...
* The number of CPU cores in `cgroup.cpuset` must be an integer multiple of the corresponding number of CPU caches

According to [rust-fil-proofs](https://github.com/filecoin-project/rust-fil-proofs), when `multicore sdr` is enabled, the number of CPU cores and number of corresponding CPU caches must be an integer multiple relationship. If the external processor is using rust-fil-proofs or rust-fil-proofs based variants, this rule must be followed, otherwise it can be ignored. The default processor of venus-worker is based on rust-fil-proofs.
...
Use [venus-worker-util](https://github.com/ipfs-force-community/venus-cluster/blob/main/docs/en/12.venus-worker-util.md#hwinfo) to check CPU information
```shell
...
At this time, the number of CPU caches is 2 (L3#0, L3#1), the number of CPU cores is 7. The number of CPU cores is not an integer multiple of the corresponding number of CPU caches, which means that it does not meet the rust-fil-proofs rule, thus cannot be started. The correct configuration could be `processors.pc1.cgroup.cpuset = "0-7"`.
