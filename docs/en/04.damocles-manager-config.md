...
`venus-cluster` is designed to support multiple `SP`s with the same set of components. This is reflected in `damocles-manager`, which you can set multiple `Miners` configuration blocks as needed.
