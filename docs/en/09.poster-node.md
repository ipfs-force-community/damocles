...
#####  An example of using one graphics card to do two wdpost tasks at the same time
...
concurrent = 1
```
The significance of single graphics card doing two wdpost tasks at the same time is that because the graphics card is not used when calculating vanilla_proofs, the vanilla_proofs of the two wdpost tasks can be calculated in parallel, and the gpu lock file in `TMPDIR` is used to serially calculate `snark_proof`. GPU utilization can be greatly improved.
