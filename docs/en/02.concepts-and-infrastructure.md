...
This is a set of storage order-related interface definitions defined in `venus/venus-shared`. It mainly provides Damocles with basic capabilities for order allocation and obtaining order data.
...
This module manages orders. It is mainly responsible for assigning order `pieces` that can fit into empty allocated sectors, as well as releasing orders in failed sectors.
