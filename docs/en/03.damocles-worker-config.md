...
# If the file "/path/to/your_local_pieces_dir/piece_file_name" does not exist, the worker will also load it from the damocles-manager
...
The `sealing_thread` will inherit the configuration in [`[sealing]`](https://github.com/ipfs-force-community/damocles/blob/main/docs/en/03.damocles-worker-config.md#sealing). Any `sealing.*` in the `sealing_thread` configuration will override the configuration of `[sealing]`.
...
It is very inconvenient in some scenarios, for example: during [Sector rebuild](../zh/16.%E6%89%87%E5%8C%BA%E9%87%8D%E5%BB%BA%E7%9A%84%E6%94%AF%E6%8C%81.md) we would like to be able to reload configuration without rebooting damocles-worker after modifying the `plan` configuration item of a `sealing_thread`.
...
Due to the need for coordinating storage location information between `damocles-worker` and `damocles-manager`, in many cases the mount path for the same persistent storage directory on the `damocles-worker` machine and on the `damocles-manager` are not exactly the same, so we decided to use `name` as the basis for coordination.

You can also choose not to configure `name` on both sides of `damocles-worker` and `damocles-manager` during configuration if the mount path of the persistent storage directory is the same on all machines. In this case, both will use the absolute path as the `name`, which will also match.
...
# Default is false
# Starting from v0.4.0, selection strategy related config will be relocated to damocles-managergr; this config on woker is deprecated
# enable_space_weighted = false
```
...
* The CPU cores in `cgroup.cpuset` should not span NUMA nodes as much as possible. Across NUMA nodes will make CPU access to memory slower. (**`damocles` [supports loading NUMA affinity `hugepage` memory files](../zh/15.damocles-worker_PC1_HugeTLB_Pages_支持.md) after v0.5.0, if this feature is enabled then you can allocate `cpusets` across NUMA nodes without concerns**)
