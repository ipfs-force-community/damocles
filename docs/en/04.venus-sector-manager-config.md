...
# default is null
# When not set, it is regarded as allowing all miner numbers; when set, it is equivalent to a whitelist, which is only allowed to be assigned to the listed miner numbers
