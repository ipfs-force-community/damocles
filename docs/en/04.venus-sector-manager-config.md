...
# Status polling interval, optional, event type
# Default is 3min
#PollInterval = "3m0s"

# API interface exception retry interval, optional, event type
# Default is 3min
#APIFailureWait = "3m0s"

# Retry interval for local exceptions, such as local database exceptions, local storage exceptions, etc., optional, event type
# Default is 3min
#LocalFailureWait = "3m0s"
````
...
````
[Miners.Post]
# Sender address, required items, address type
Sender = "t1abjxfbp274xpdqcpuaykwkfb43omjotacm2p3za"
...
- Enable sector allocation, which initial number is 1000
- Disable aggregated PreCommit
