...
### [Common.Plugins]
`Common.Plugins` is used for configuring the plugin path of venus-sector-manager.
...
# The probability of each persistent stores being selected is `weight / s`, where `s` is the sum of the weights of all persistent stores.
