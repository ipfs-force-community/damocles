...
2. 创建一个普通的 go package，并创建 `manifest.toml` 文件。
	...
	`manifest.toml` 文件的内容会被解析为 `damocles-manager-plugin/spi.go` 中的 `{Kind}Manifest` 结构体。

	[manifest.toml example:](https://github.com/ipfs-force-community/damocles/blob/dfc20a9a4d2728192bbbf830ddfd15b684b98ce9/damocles-manager/plugin/examples/memdb/manifest.toml#L3-L10)
	```toml
	# manifest.toml
...
##### Example: [fsstore](https://github.com/ipfs-force-community/damocles/tree/main/damocles-manager-plugin/examples/fsstore)
...
##### Example: [memdb](https://github.com/ipfs-force-community/damocles/tree/main/damocles-manager-plugin/examples/memdb)
